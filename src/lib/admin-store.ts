...
const KEYS = {
  ...
  BLOG_POSTS: "ambika_blog_posts",
  BLOG_SEEDED: "ambika_blog_seeded",
};
...
<p>In the weaving clusters of Sonepur and Bargarh in western Odisha, something remarkable happens every morning before dawn. Master artisans begin a process that transforms raw silk threads into mathematical masterpieces — Sambalpuri Ikat sarees.</p>
...
/** Get all blog posts (sync — from localStorage cache) */
export function getBlogPosts(): BlogPost[] {
  ensureBlogSeeded();
  return lsGet<BlogPost[]>(KEYS.BLOG_POSTS, SEED_BLOG_POSTS);
}

/** Get all blog posts (async — tries Supabase first) */
export async function getBlogPostsAsync(): Promise<BlogPost[]> {
  if (isSupabaseConfigured()) {
    try {
      const { data, error } = await supabase!
        .from("blog_posts")
...
export async function saveBlogPost(post: BlogPost): Promise<void> {
  if (isSupabaseConfigured()) {
    try {
      ...
      console.error("Blog Supabase save error:", err);
    }
  }
...
export async function deleteBlogPost(id: string): Promise<void> {
  if (isSupabaseConfigured()) {
    try {
      ...
      console.error("Blog Supabase delete error:", err);
    }
  }
...
function mapDbBlogPost(row: any): BlogPost {
  return {
    ...
    tags: row.tags || [],
    author: row.author || "Ambika Handloom",
